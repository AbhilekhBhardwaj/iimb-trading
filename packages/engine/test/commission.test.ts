...
describe('commissionInrFor', () => {
  it('is rate × notional × fx when the round has commission enabled', () => {
    ...
    expect(commissionInrFor(10, 190, RATE, true)).toBeCloseTo(473.1, 6)
    expect(commissionInrFor(10, 190, RATE, true)).toBeCloseTo(COMMISSION_RATE * 10 * 190 * RATE, 9)
  })

  it('is exactly zero when the round has commission disabled', () => {
    expect(commissionInrFor(10, 190, RATE, false)).toBe(0)
  })

  it('ignores the sign of the fill — a sell is charged like a buy', () => {
    expect(commissionInrFor(-10, 190, RATE, true)).toBe(commissionInrFor(10, 190, RATE, true))
  })

  it('scales linearly with quantity, price and rate', () => {
    expect(commissionInrFor(20, 190, RATE, true)).toBeCloseTo(2 * commissionInrFor(10, 190, RATE, true), 9)
    expect(commissionInrFor(10, 380, RATE, true)).toBeCloseTo(2 * commissionInrFor(10, 190, RATE, true), 9)
    expect(commissionInrFor(10, 190, 2 * RATE, true)).toBeCloseTo(2 * commissionInrFor(10, 190, RATE, true), 9)
  })
})

describe('closingPnlBreakdown — no breakdown when nothing is realized', () => {
  it('returns null for a fresh buy opening a new position', () => {
    expect(closingPnlBreakdown(FLAT_CASH, 10, 180, RATE, 1, true)).toBeNull()
  })
  ...
  it('returns null when adding to an existing long', () => {
    expect(closingPnlBreakdown(LONG, 5, 190, RATE, 1, true)).toBeNull()
  })

  it('returns null when adding to an existing short', () => {
    expect(closingPnlBreakdown(SHORT, -5, 170, RATE, 1, true)).toBeNull()
  })

  it('returns null regardless of the commission toggle', () => {
    expect(closingPnlBreakdown(FLAT_CASH, 10, 180, RATE, 1, false)).toBeNull()
    expect(closingPnlBreakdown(LONG, 5, 190, RATE, 1, false)).toBeNull()
  })
})

describe('closingPnlBreakdown — commission ON', () => {
  it('splits a full close into gross, commission and net', () => {
    const b = closingPnlBreakdown(LONG, -10, 190, RATE, 1, true)!
  ...
  it('realizes proportionally on a partial reduce', () => {
    const b = closingPnlBreakdown(LONG, -4, 190, RATE, 1, true)!
  ...
  it('handles a closing LOSS — commission deepens it rather than offsetting', () => {
    const b = closingPnlBreakdown(LONG, -10, 170, RATE, 1, true)!
    expect(b.grossPnlInr).toBeCloseTo(-8_300, 6)
  ...
  it('closes a SHORT at a profit when the price falls', () => {
    const b = closingPnlBreakdown(SHORT, 10, 170, RATE, 1, true)!
    expect(b.closedQty).toBe(10)
    expect(b.grossPnlInr).toBeCloseTo(8_300, 6)
  ...
  it('charges the FULL fill on a flip, while gross covers only the closed units', () => {
    ...
    const b = closingPnlBreakdown(LONG, -15, 190, RATE, 1, true)!
    expect(b.closedQty).toBe(10)
...
describe('closingPnlBreakdown — commission OFF', () => {
  it('reports zero commission and net equal to gross', () => {
    const b = closingPnlBreakdown(LONG, -10, 190, RATE, 1, false)!
    expect(b.closedQty).toBe(10)
    expect(b.grossPnlInr).toBeCloseTo(8_300, 6)
  ...
  it('leaves gross identical to the commission-ON case — only charges differ', () => {
    const on = closingPnlBreakdown(LONG, -10, 190, RATE, 1, true)!
    const off = closingPnlBreakdown(LONG, -10, 190, RATE, 1, false)!
  ...
  it('does not soften a loss either', () => {
    const b = closingPnlBreakdown(LONG, -10, 170, RATE, 1, false)!
    expect(b.grossPnlInr).toBeCloseTo(-8_300, 6)
...
describe('closingPnlBreakdown — leverage and rate', () => {
  it('P&L is on full notional, so leverage does not change gross or net', () => {
    ...
    const flat = closingPnlBreakdown(LONG, -10, 190, RATE, 1, true)!
    const lev = closingPnlBreakdown(levered, -10, 190, RATE, 5, true)!
  ...
  it('a pure FX move realizes P&L even at an unchanged USD price', () => {
    const b = closingPnlBreakdown(LONG, -10, 180, 90, 1, true)!
    // basis ₹14,940/unit vs exit 180 × 90 = ₹16,200/unit → ₹1,260 × 10
    expect(b.grossPnlInr).toBeCloseTo(12_600, 6)
    expect(b.commissionInr).toBeCloseTo(COMMISSION_RATE * 10 * 180 * 90, 9)
  })
})
